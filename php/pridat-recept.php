...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errors === []) {
        ...
        if ($isAdmin) {
            // Admin: přímé zveřejnění
            header('Location: recept-pridan.php?slug=' . urlencode($slug));
        } else {
            // Uživatel: pošli e-mail ke schválení
            $token = bin2hex(random_bytes(32));
            $recipeRepo->requestApproval($newRecipe->id, $token);

            $protocol    = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off') ? 'https' : 'http';
            $dir         = rtrim(dirname($_SERVER['PHP_SELF']), '/');
            $approvalUrl = $protocol . '://' . $_SERVER['HTTP_HOST'] . $dir . '/schvalit-recept.php?token=' . urlencode($token);

            Mailer::sendApproval($name, $currentUser->email, $approvalUrl);

            $_SESSION['flash'] = 'Recept byl odeslán ke schválení. Zobrazí se po schválení administrátorem.';
...
$pageDescription = 'Přidejte nový recept do Kottyho kuchařky. Vyplňte název, ingredience, postup a obrázek.';
$favoritesCount  = $favorites->count();

require __DIR__ . '/views/pridat-recept.php';
