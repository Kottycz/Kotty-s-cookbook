...
// Pouze majitel nebo admin může recept upravovat
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($errors === []) {
        ...
        if ($ingLines !== []) $recipeRepo->replaceIngredients($recipe->id, $ingLines);
        if ($stepLines !== []) $recipeRepo->replaceSteps($recipe->id, $stepLines);

        $_SESSION['flash'] = 'Recept byl upraven.';
        header('Location: moje-recepty.php');
        exit;
    }
}
...
$pageDescription = 'Upravte recept ' . $recipe->name . ' ve vaší osobní knihovně na Kottyho kuchařce.';
$favoritesCount  = $favorites->count();

require __DIR__ . '/views/upravit-muj-recept.php';
